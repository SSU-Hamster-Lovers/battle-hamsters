# 미니 스펙: 무기 확장 v3 (11~13번째 무기) — **완료**

> **상태**: `feat/weapon-expansion-v3` 브랜치에서 구현 완료. 누적 무기 13종, 서버 단위 테스트 71개.
...
현재 `pinecone_grenade` 폭발은 `blueberry_mortar`와 동일한 `spawnExplosionBurst` 파티클을 재사용한다. 시각적 다양성 부족.

**개선 방향**: 무기 VFX 일괄 개선 미니 스펙(`mini-spec-vfx-overhaul-v1.md`) 작성 시 처리.
- 수류탄 전용 파티클: 갈색/황갈색 + 더 큰 폭발 반지름
- 폭발 음파 효과(충격파 원 애니메이션)
- 퓨즈 점화 파티클 (투사체 비행 중 스파크)
